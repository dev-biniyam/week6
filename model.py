...
class Model:
    ...
    def train_logistic_classifier(self, X_train, y_train):
        #initialize logistic regression from sklearn
        log_regression = LogisticRegression()

        #fit the training data into the model and get accuracy scores
        log_regression = self.train_model(log_regression, X_train, y_train)
    ...
    def train_xgboost(self, X_train, y_train):
        ...
        xg_boost = self.train_model(xg_boost, X_train, y_train)
        return xg_boost


    def train_perceptrons(self, X_train, y_train):
        #initialize Multilayer perceptron classifier from sklearn
        clf = MLPClassifier(solver='lbfgs', alpha=1e-5,hidden_layer_sizes=(5, 2), random_state=1)

        #fit the data and get accuracy scores
        clf = self.train_model(clf, X_train, y_train)
        return clf
